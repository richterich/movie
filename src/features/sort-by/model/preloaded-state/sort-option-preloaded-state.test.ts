...
describe('[features/sort-by] option preloaded state', () => {
  ...
  test('should return the correct sort option from URLSearchParams', () => {
    const searchParams = new URLSearchParams(`sort.by=${OptionSortParams.ReleaseDate}`);
    expect(sortOptionPreloadedState(searchParams)).toBe(SortOptions.ReleaseDate);
  });

  test('should handle case-insensitive sort option from URLSearchParams', () => {
    const searchParams = new URLSearchParams(`sort.by=${OptionSortParams.ReleaseDate.toUpperCase()}`);
    expect(sortOptionPreloadedState(searchParams)).toBe(SortOptions.ReleaseDate);
  });
