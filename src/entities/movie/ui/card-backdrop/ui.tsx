...
export const BackdropCard = ({movie}: Props) => {
  return (
    <Link className="relative h-full w-full" to={paths.movieDetails(movie.id)}>
      <div className="relative aspect-video bg-[#111] after:absolute after:inset-0 after:bg-black/25 after:opacity-0 after:transition-opacity after:duration-300 after:hover:opacity-100">
        <LazyImage
